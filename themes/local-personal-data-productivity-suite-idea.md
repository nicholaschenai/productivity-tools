...
- Ideas and features:
    ...
    - Reconstruct the day automatically from passive signals (e.g. active-window titles, wearable heart
        rate and steps, calendar, commits, expense timestamps, voice notes) so manual logging is reduced. 
        The user corrects errors rather than entering everything.
    ...
    - Reflection dashboards at different timescales, with drill-down from a macro summary to minute-level detail.
...
- Ideas and features:
    ...
    - Recommendation system: filter the task pool by the metrics above, because
        what counts as doable fluctuates through the day, e.g. it depends on a person's available slots, cognitive types exercised in the day (to avoid straining too much of any one mode of cognition) etc
    ...
    - Verifiers that auto-complete quests from real signals (fitness data, commits) for reduced friction
...
- Ideas and features:
    - Build a searchable knowledge base from personal notes, reflections, and lesson documents, and
        return relevant advice for a query, a daily goal, or recent activity.
    ...
    - Contextual retrieval: derive the conditions under which a lesson applies (a book's advice is
        situational) and match those conditions to the current situation, not just surface similarity.
...
- Ideas and features:
    ...
    - Verify habits automatically from various signals (e.g. activity trackers, computers) and visualise adherence, highlighting the ones being missed.
